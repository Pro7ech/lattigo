...
// FailureProbability returns the probability that PR[||I(X)|| > K].
...
func FailureProbability(Xs *ring.Ternary, K, logN, logSlots int) (logfailure float64) {
	...
	if Xs.P != 0 {
		...
		for step != 0 {
			...
			d := float64(c) / (kappa * s)
			erfc := bignum.Log2ErfC(d/sqrt2, prec)
...
func FindSuitableK(Xs *ring.Ternary, logN, logSlots int, logfailure float64) (K int) {
	...
	getCorrectionFactor := func(logfailure float64) (d float64) {
		...
		d = 1.0
		for bignum.Log2ErfC(d/sqrt2, prec) > logfailure {
			d++
		}
		...
		for step > 1e-3 {
			...
			if bignum.Log2ErfC(d/sqrt2, prec) > logfailure {
				d += step
			} else {
				d -= step
			}
