...
func main() {
	...
	btpParametersLit.LogN = params.LogN() // Ensures fast test because default value is 16
	btpParametersLit.LogSlots = params.LogN() - 1
	...
	if *flagShort {
		...
		btpParams.Mod1.LogMessageRatio += 16 - params.LogN()
	}
	...
	for i := range valuesWant {
		valuesWant[i] = source.Complex128(-1-1i, 1+1i)
	}
	...
	valuesTest1 := printDebug(params, ciphertext, valuesWant, decryptor, encoder)
	...
	now := time.Now()
	out, err := eval.Bootstrap(ciphertext)
	if err != nil {
		panic(err)
	}
	...
	printDebug(params, out, valuesTest1, decryptor, encoder)
}
