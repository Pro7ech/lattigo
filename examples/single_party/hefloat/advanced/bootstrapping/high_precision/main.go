...
func main() {
	...
	btpParametersLit := bootstrapping.NewParametersLiteral()
	btpParametersLit.LogN = LogN
	btpParametersLit.LogSlots = LogN - 1
	...
	btpParametersLit.Iterations.BootstrappingPrecision = []float64{25, 25, 5}
	btpParametersLit.Iterations.ReservedPrimeBitSize = 28

	if *flagShort {
		// Corrects the message ratio Q0/|m(X)| to take into account the smaller number of slots and keep the same precision
		btpParametersLit.LogMessageRatio += 16 - params.LogN()
	}
	...
	for i := range valuesWant {
		valuesWant[i].SetPrec(prec)
		valuesWant[i][0].SetFloat64(source.Float64(-1, 1))
		valuesWant[i][1].SetFloat64(source.Float64(-1, 1))
	}
	...
	// Encrypt
	ciphertext1 := hefloat.NewCiphertext(params, 1, plaintext.Level())
	if err := encryptor.Encrypt(plaintext, ciphertext1); err != nil {
		panic(err)
	}
...
func printDebug(params hefloat.Parameters, ciphertext *rlwe.Ciphertext, valuesWant []bignum.Complex, decryptor *rlwe.Decryptor, encoder *hefloat.Encoder) (valuesTest []bignum.Complex) {
	...
	fmt.Printf("ValuesTest: %6.27f %6.27f, %6.27f %6.27f...\n", &valuesTest[0][0], &valuesTest[0][1], &valuesTest[1][0], &valuesTest[1][1])
	fmt.Printf("ValuesWant: %6.27f %6.27f, %6.27f %6.27f...\n", &valuesWant[0][0], &valuesWant[0][1], &valuesWant[1][0], &valuesWant[1][1])
