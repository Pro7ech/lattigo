...
func main() {
	...
	evkParams.DigitDecomposition.Type = rlwe.Signed
	evkParams.Log2Basis = 12
	...
	testPolyMap := make(map[int]*ring.RNSPoly) // Which slot to evaluate on the BlindRotation
	...
	ctN12 := rlwe.NewCiphertext(paramsN12, 1, pt.Level(), -1)
	if err := encryptorN12.Encrypt(pt, ctN12); err != nil {
		panic(err)
	}
	...
	buf := evalHDFT.NewHoistingBuffer(ctN12.Level(), paramsN12.MaxLevelP())

	// Homomorphic Decoding: [(a+bi), (c+di)] -> [a, c, b, d]
	ctN12, err = evalHDFT.SlotsToCoeffsNew(ctN12, nil, SlotsToCoeffsMatrix, buf)
	if err != nil {
		panic(err)
	}
	...
	ctN12, _, err = evalHDFT.CoeffsToSlotsNew(ctN12, CoeffsToSlotsMatrix, buf)
	if err != nil {
		panic(err)
	}
	fmt.Printf("Done (%s)\n", time.Since(now))
